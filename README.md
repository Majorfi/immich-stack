...
- **Configurable Grouping:** Supports custom grouping logic via environment variables and JSON criteria.
...
- **Custom Criteria:** Override with the `CRITERIA` environment variable (JSON array of criteria).

### Sorting

- **Parent Promotion:** Use `PARENT_FILENAME_PROMOTE` (comma-separated substrings) to promote files as stack parents.
- **Extension Promotion:** Use `PARENT_EXT_PROMOTE` (comma-separated extensions) to further prioritize.
...
With `PARENT_FILENAME_PROMOTE=edit` and `PARENT_EXT_PROMOTE=.jpg,.dng`, the order will be:
...
- **Stacker:** Main struct for stacking logic.
- **ApplyCriteria:** Applies grouping criteria to assets.
- **StackBy:** Groups assets into stacks.
...
- **Custom Grouping:** Edit or override `DefaultCriteria` in `pkg/stacker/types.go` or set the `CRITERIA` env var.
- **Custom Promotion:** Set `PARENT_FILENAME_PROMOTE` and/or `PARENT_EXT_PROMOTE` for your workflow.
