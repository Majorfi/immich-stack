...
Identifies trashed assets and moves their related stack members to trash for consistency. With `--trash-orphaned-raws`, it also removes RAW files left without a developed companion (JPG, HEIC, ...). Set `FIX_TRASH_AFTER_STACKING=true` to chain it after every stacking run, including in cron mode.
