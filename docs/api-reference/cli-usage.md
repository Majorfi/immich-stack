...
- **fix-trash**: Uses global flags plus the stacking criteria flags (`--criteria`, `--parent-filename-promote`, etc.) to determine which assets to move to trash; `--trash-orphaned-raws` additionally enables the orphaned RAW cleanup pass, scoped by `--raw-orphan-extensions`
